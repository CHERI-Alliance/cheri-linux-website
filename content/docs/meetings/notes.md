 ...
 * Hesham and Jess argued that pointer hashing and ASLR should be disabled by default on CHERI systems, as CHERI already provides protection against the attack classes that these mitigations target, making them redundant while still incurring their associated costs.
 ...
 * Christian would like to keep the warning but agreed to reduce its prominence and turn into an informational message.
 * Whether pointer hashing and other mitigations that overlap with CHERI should be turned on by default is still open to debate. Christian noted that leaked kernel addresses could potentially be used for attacks even in the presence of CHERI, in ways that the WG is not aware of. He mentioned speculative execution attacks as an example. He also noted that the likelihood of patch acceptance during future upstreaming efforts should be considered and that some like to layer mitigations on top of each other for defense in depth.
